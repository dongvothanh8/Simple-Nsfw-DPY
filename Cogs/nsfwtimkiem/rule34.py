...
class nsfwtimkiem(commands.Cog):
  def __init__ (self, bot):
    self.bot = bot 
  @commands.command(pass_context=True, no_pm=True, aliases=['r34'])
  @commands.is_nsfw()
  async def rule34(self, ctx, *, message:str):
    async with ctx.typing():
        await asyncio.sleep(1)
    limit = 10000
    message = message.replace(" ", "_")
    url = "http://api.rule34.xxx/index.php?page=dapi&s=post&q=index&json=1&limit={}&tags={}".format(limit, message)
    try:
  ...
  async def rule34_error(self, ctx: commands.Context, error):
    ...
    if isinstance(error, commands.errors.MissingRequiredArgument):
      return await ctx.reply("**Mrule34 <cái củ lol gì đó>\n VD: Mrule34 mikoto**", delete_after=5)
